...
> 
> The Python `for` loop is by default an enhanced or foreach loop over a traditional c-style for-loop. C++ & Java programmers will note that Python's for is the same as their `for (int i : IntArray)` versions and especially the `for (var i : IntArray)` of Java 11+ or the `for (const auto& i : IntArray)` of Modern C++ (11+).
> 
